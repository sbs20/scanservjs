...
describe('ScanimageCommand', () => {
  ...
  it('scanimage-a10.txt', () => {
    ...
    assert.strictEqual(command, `/usr/bin/scanimage -d epjitsu:libusb:001:003 --source 'ADF Front' --mode Color --resolution 300 --page-width 215.8 --page-height 292 -t 0 --format tiff --brightness 0 --contrast 0 -o temp/~tmp-scan-0-0001.tif`);
  });
  ...
  it('scanimage-a14.txt', () => {
    ...
    // eslint-disable-next-line quotes
    assert.strictEqual(command, `/usr/bin/scanimage -d 'fujitsu:ScanSnap S1500:8176' --source 'ADF Front' --mode Lineart --resolution 600 --page-width 215.8 --page-height 279.3 -l 0 -t 0 -x 215.8 -y 279.3 --format tiff --ald=yes --brightness 0 -o temp/~tmp-scan-0-0001.tif`);
  });
  ...
  it('scanimage-batch.txt', () => {
    ...
    const command = commandFor('1.1.1', request);
    // eslint-disable-next-line quotes
    assert.strictEqual(command, `/usr/bin/scanimage -d 'pixma:04A91766_004AE4' --source 'Automatic Document Feeder' --mode Color --resolution 75 -l 0 -t 0 -x 216 -y 355.6 --format tiff --batch='temp/~tmp-scan-1-%04d.tif'`);
  });
